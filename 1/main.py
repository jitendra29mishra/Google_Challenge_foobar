...
def answer(s):
    s_length = len(s)
    factors_of_s_length = [i for i in range(1, 1 + s_length//2) if s_length%i == 0]

    for i in factors_of_s_length:
        s_pieces = [s[posi: posi+i] for posi in range(0, s_length, i)]
        print(s_pieces)
        if all([s_pieces[0] == piece for piece in s_pieces]):
